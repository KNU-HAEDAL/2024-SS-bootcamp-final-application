...
def get_category_headlines(category: str):
    ...
    response = requests.get(f"{base_url}{category_code}", headers=headers) #f"{base_url}{category_code}"==https://news.naver.com/section/101
    ...
    for item in news_items:
        title = item.find('strong', class_='sa_text_strong').text.strip() #strip : 공백 없애줌
